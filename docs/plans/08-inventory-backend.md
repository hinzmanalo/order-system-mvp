...
- [x] `com.orderhub.inventory.entity.Inventory.java`:
...
- [x] `com.orderhub.inventory.repository.InventoryRepository.java`:
...
- [x] `InventoryResponse.java` — productId, productName, productSku, quantity, updatedAt
- [x] `SetStockRequest.java` — `@NotNull @Min(0)` quantity (Integer)
- [x] `AdjustStockRequest.java` — `@NotNull` adjustment (Integer, can be negative)

### 8.4 Service

- [x] `com.orderhub.inventory.service.InventoryService.java` — interface:
...
- [x] `com.orderhub.inventory.service.InventoryServiceImpl.java`:
...
- [x] `com.orderhub.inventory.controller.AdminInventoryController.java`:
...
- [x] `GET /api/v1/admin/inventory` as ADMIN returns paginated inventory list with product details
- [x] `GET /api/v1/admin/inventory` as USER → 403
- [x] `GET /api/v1/admin/inventory/{productId}` returns single inventory record
- [x] `PUT /api/v1/admin/inventory/{productId}` with `{"quantity": 100}` sets stock to 100
- [x] `PUT /api/v1/admin/inventory/{productId}` with `{"quantity": -1}` → 400 (validation)
- [x] `PATCH /api/v1/admin/inventory/{productId}/adjust` with `{"adjustment": -5}` reduces stock by 5
- [x] `PATCH /api/v1/admin/inventory/{productId}/adjust` with delta that would go below 0 → 409
- [x] Concurrent updates: two simultaneous requests to same product → one succeeds, other gets 409
